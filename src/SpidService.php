...
//      'sp_contact_ipa_code' => 'xxx',
//      'sp_contact_fiscal_code' => 'xxx',
//      'sp_contact_email' => 'xxx',
//      'sp_contact_phone' => 'xxx',
    ...
    $this->spid = new Sp($settings, 'saml', FALSE);
  }
    ...
    try {
      if ($this->spid->isAuthenticated()) {
        ...
        if (!$account) {
          ...
          $this->eventDispatcher->dispatch($event, spidEvents::USER_LINK);
  ...
  public function synchronizeUserAttributes(UserInterface $account, $skip_save = FALSE) {
    ...
    $this->eventDispatcher->dispatch($event, spidEvents::USER_SYNC);
